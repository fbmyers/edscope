...
//
//  Modified by Frankie Myers
//  --removed releases (for ARC)
//  --added explicit bridged casting from CF to Obj C
//  --added CellScope metadata function
...
//this field gets picked up by Flickr as title
- (void)setTitle:(NSString*)title {
    [IPTC_DICT setObject:title forKey:(NSString*)kCGImagePropertyIPTCHeadline]; //FBM
}

//this field gets picked up by Flickr as description
- (void)setDescription:(NSString*)description {
    //[TIFF_DICT setObject:description forKey:(NSString*)kCGImagePropertyTIFFImageDescription];
    [IPTC_DICT setObject:description forKey:(NSString*)kCGImagePropertyIPTCCaptionAbstract]; //FBM
}

//keywords should be a \n delimited list for Flickr to separate them
...
- (void)setCellScopeVersion:(NSString*)softwareVersion
                studentName:(NSString*)studentName
                  groupName:(NSString*)groupName
               locationName:(NSString*)locationName
                cellScopeID:(NSString*)cellScopeID
              magnification:(NSString*)magnification
              ...
              pictureNumber:(long)pictureNumber
{
    ...
    //I'm not sure if these get saved to the JPEG at all, can't find them with exiftool
    //might be able to find analogous tags with EXIF and use those instead
    [self setObject:softwareVersion forKey:@"CellScopeSofwareVersion"];
    [self setObject:studentName forKey:@"CellScopeUserName"];
    [self setObject:groupName forKey:@"CellScopeGroupName"];
    [self setObject:locationName forKey:@"CellScopeLocationName"];
    [self setObject:cellScopeID forKey:@"CellScopeID"];
    [self setObject:magnification forKey:@"CellScopeMagnification"];
    [self setObject:[NSString stringWithFormat:@"%3.3f",pixelsPerMM] forKey:@"CellScopePixelsPerMM"];
    [self setObject:[NSString stringWithFormat:@"%ld",pictureNumber] forKey:@"CellScopePictureNumber"];
