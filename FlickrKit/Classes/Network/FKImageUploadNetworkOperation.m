...
@property (nonatomic, strong) NSData* imageData; //FBM
@property (nonatomic) BOOL uploadFromUIImage; //FBM
...
- (id) initWithImage:(UIImage *)image arguments:(NSDictionary *)args completion:(FKAPIImageUploadCompletion)completion; {
    ...
    if (self) {
        ...
        self.uploadFromUIImage = YES; //FBM
    }
    return self;
}

//FBM
- (id) initWithData:(NSData *)imageData arguments:(NSDictionary *)args completion:(FKAPIImageUploadCompletion)completion; {
    self = [super init];
    if (self) {
		self.imageData = imageData;
		self.args = args;
		self.completion = completion;
        self.uploadFromUIImage = NO;
    }
    return self;
}
...
//#endif
    ...
    //FBM added...
    NSInputStream* inImageStream;
    if (self.uploadFromUIImage) //original method
    {
        NSData *imgData = UIImageJPEGRepresentation(self.image, 1.0);
        inImageStream = [[NSInputStream alloc] initWithData:imgData];
	}
    else //alternative way to do this, upload the file as NSData, thereby preserving Exif data
    {
        inImageStream = [[NSInputStream alloc] initWithData:self.imageData];
    }
